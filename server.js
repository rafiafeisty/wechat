...
// Delete chat instance for all participants
app.delete("/api/chat/delete-chat", authenticateUser, async (req, res) => {
  try {
    const { contactEmail } = req.body;
    const userEmail = req.user.email;
    ...
    const participants = [userEmail, contactEmail].sort();
    await Chat.deleteOne({ participants: { $all: participants } });
...
// Delete all records including contact and chat
app.delete("/api/chat/delete-all-records", authenticateUser, async (req, res) => {
  try {
    const { contactEmail } = req.body;
    const userEmail = req.user.email;
    ...
    // Delete contact relationship both ways
